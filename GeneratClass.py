...
# generate header file
    with open(className + '.hpp', 'w') as f:
        ...
        headerFile += "\t" + "~" + className + "();\n"
        headerFile += "\t" + className + "(const " + className + " &" + className[0].lower() + ");\n"
        headerFile += "\t" + className + " & operator =" + "(const " + className + " &" + className[0].lower() + ");\n"
        headerFile += "private: \n\n"
        headerFile += "}; \n\n"
...
# generate cpp file
    with open(className + '.cpp', 'w') as f:
        ...
        cppFile += "} \n\n"
        cppFile += className + "::" + className + "(const " + className + " &" + className[0].lower() + "){\n"
        cppFile += "\t" + "std::cout << \"Copy constructor called\" << std::endl;\n"
        cppFile += "\t *this = " + className[0].lower() + ";\n"
        cppFile += "} \n\n"
        cppFile += className + " & "  + className + "::operator =" + "(const " + className + " &" + className[0].lower() + "){\n"
